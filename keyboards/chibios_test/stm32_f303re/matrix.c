...
static void select_row(uint8_t row, uint8_t state);
...
#define LED_ON()    do { palSetPad(GPIOA, 5) ;} while (0)
#define LED_OFF()   do { palClearPad(GPIOA, 5); } while (0)
#define LED_TGL()   do { palTogglePad(GPIOA, 5-); } while (0)
...
static matrix_row_t read_cols(void)
{
	return ((palReadPad(GPIOA, 0)==PAL_LOW) ? 0 : (1<<0)) |
			((palReadPad(GPIOA, 1)==PAL_LOW) ? 0 : (1<<1)) |
			((palReadPad(GPIOA, 4)==PAL_LOW) ? 0 : (1<<2)) |
			((palReadPad(GPIOB, 0)==PAL_LOW) ? 0 : (1<<3)) |
			((palReadPad(GPIOC, 1)==PAL_LOW) ? 0 : (1<<4));
}
...
static void select_row(uint8_t row, uint8_t state)
{
	...
	switch (row) {
	case 0:
		state ? palSetPad(GPIOB, 15) : palClearPad(GPIOB, 15);
		break;
	case 1:
		state ? palSetPad(GPIOB, 14) : palClearPad(GPIOB, 14);
		break;
	case 2:
		state ? palSetPad(GPIOB, 13) : palClearPad(GPIOB, 13);
		break;
	default:
		break;
	};
